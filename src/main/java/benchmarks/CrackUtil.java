...
// Logique de cassage partagee entre le mode local (BruteForceGeneratorParallel) et le
// mode distribue (DistributedMaster/DistributedWorker) : alphabet, comparaison binaire
// 64 bits, et le scan exhaustif d'un bloc (longueur + 1er caractere fixe).
final class CrackUtil {
    ...
    // Scanne exhaustivement un bloc (candidats de `length` caracteres commencant par
    // `firstChar`) ou s'arrete des que `found` est trouve. Retourne le mot trouve (ou null)
    // et le nombre de tentatives effectuees dans ce bloc.
